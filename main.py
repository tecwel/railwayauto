...
CSV_FILE = "data.csv"
CSV_HEADERS = ["Period", "Number", "Premium"]
MAX_ENTRIES = 8000  # Maximum data rows (excluding header)
...
if creds_json:
    ...
    # ✅ Configure GoogleAuth to skip client_secrets.json
    gauth = GoogleAuth(settings={
        "client_config_backend": "service",
        "service_config": {
            "client_json_file_path": creds_path
...
def write_to_csv(items):
    ...
    # Combine with new data at the top
    combined_data = new_data + existing_data

    # Trim to 8000 if necessary
    if len(combined_data) > MAX_ENTRIES:
        print("🧹 Trimming CSV to 8000 rows.")
        combined_data = combined_data[:MAX_ENTRIES]
